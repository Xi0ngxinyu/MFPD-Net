...
parser.add_argument('--dataset', type=str, default='D:\learn\dehazeDataset\SOTS', help='Path of the validation dataset')
parser.add_argument("--checkpoint", default=r"", type=str, help="Test on intermediate pkl (default: none)")
...
def test(test_gen, model, criterion, SR_dir):
    ...
    with torch.no_grad():
        for iteration, batch in enumerate(test_gen, 1):
            # if iteration==590 or (iteration>=391 and iteration<=400):
            # print(iteration)
            Blur = batch[0]
            HR = batch[1]
            Blur = Blur.to(device)
